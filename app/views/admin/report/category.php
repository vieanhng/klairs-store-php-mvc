...
<?php
$report = $data['report'];
$reportCats = $data['reportCats'];
$count = $data['reportCats'] ? count($data['reportCats']) : 0;
    $page = isset($_GET['page']) ? $_GET['page'] : 1;
    if($reportCats){
        $records_per_page = 10;
        $pagination = new Zebra_Pagination();
        $pagination->records(count($reportCats));
        $pagination->records_per_page($records_per_page);
        $reportCats = array_slice(
            $reportCats,
        ...
        <div class="card-body">
            <form class="text-dark">
                ...
                <button class="btn btn-success ml-3" type="submit" style="margin-right: 20px;">Xuất Excel</button>
                    <a href="<?=getUrl('admin/report/categories')?>"><button class="btn btn-secondary" type="button" style="margin-right: 20px;">Làm mới</button></a>
        ...
        <div class="card-title">   
           <!-- <div class="row" style="background: #ffffff;margin-left: -12px;"> .-->
                <div class="col-md-12" style=" margin-left: 18px;padding-top: 20px;height: 40px;">
                    <p class="fw-bold">BÁO CÁO THEO DANH MỤC</p>
                </div>
        </div>
        <div class="card-body">             
           ...
           <!-- <div class="row" style="margin-left: 12px;margin-right: 12px; margin-top: 30px;">   -->
                <div class="table-responsive" style="border: 0.5px solid rgb(227,227,227);">
                    <table class="table table-borderless">
                        ...
                        <tbody>
                        <?php  if ($count): ?>
                        <?php  foreach ($reportCats as $index => $reportCat):?>
                            <tr>
                                <td><?= $index + 1 + 10*($page-1)?></td>
                                <td><?=$reportCat->ma_danh_muc?></td>
                                <td><?=$reportCat->ten_danh_muc?></td>
                                <td><?=$reportCat->so_luong_ban?></td>
                                <td><?=formatPrice($reportCat->doanh_thu)?></td>
                                <td><?=formatPrice($reportCat->loi_nhuan)?></td>                          
                            </tr>
                        <?php endforeach;?>
                        <?php endif;?>
                        </tbody>   
